...
export class InvoiceIndexComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private invoiceService: SalesReportInvoiceService,
  ...
  getDatatable() {
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.invoiceService.indexDatatables(dataTablesParameters, this.dataFormFilter)
      ...
      columns: [
        ...
        {title: 'Date', data: 'invoice_date', orderable: true},
        ...
        {title: 'Invoice No.', data: 'code', orderable: true},
        // {title: 'Due Date', data: 'payment_due_date', orderable: true},
        {title: 'Status', data: 'status', orderable: false},
        {title: 'Sales Rep', data: 'sales_rep', orderable: false},
      ],
    };
  }
  ...
  print() {
    ...
    for (x of this.datasets) {
      ...
      row += '<tr>';
      row += '<td>' + i + '</td>';
      row += '<td>' + x.invoice_date + '</td>';
      row += '<td>' + x.branch_name + '</td>';
      row += '<td>' + x.division_name + '</td>';
      row += '<td>' + x.customer.name + '</td>';
      row += '<td>' + x.customer_code + '</td>';
      row += '<td>' + x.sales_order_code + '</td>';
      row += '<td>' + x.code + '</td>';
      row += '<td>' + x.payment_due_date + '</td>';
      row += '<td>' + x.status + '</td>';
      row += '<td>' + ((x.user.length == 0 || x.user == '') ? '---' : x.user.employee_name) + '</td>';
      row += '</tr>';
      table += row;
    }
    ...
    popupWin.document.write(`
      <html>
        ...
        <body>
          <h1 class="page-header">Report Invoice</h1>
  ...
  export() {
    ...
    for (x of this.datasets) {
      ...
      row = {
        ...
        'Division': x.division_name,
        'Customer': x.customer.name,
        'Customer Code': x.customer_code,
        'Sales Order No.': x.sales_order_code,
        'Invoice No.': x.code,
        'Due Date': x.payment_due_date,
        'Status': x.status,
        'Sales Rep.': ((x.user.length == 0 || x.user == '') ? '---' : x.user.employee_name),
      };
      data.push(row);
    }
    this.excelService.exportAsExcelFile(data, 'invoice');
  }

  dataFilter() {
    const activeModal = this.modalService.open(FilterInvoiceComponent, {
      size: 'lg',
      container: 'nb-layout',
      backdrop: 'static',
    });
  ...
  exportExcel() {
    ...
    this.invoiceService.exportExcel(this.dtParams, this.dataFormFilter).subscribe(data => {
        fileSaver.saveAs(data, 'invoice-report.xlsx');
  ...
  exportPdf() {
    this.toasterService.popAsync('info', 'Start Download', 'Generating, Pleasewait...')
    this.invoiceService.exportPdf(this.dtParams, this.dataFormFilter).subscribe(data => {
        console.info(`pdf data: ${data}`);
        fileSaver.saveAs(data, 'invoice-report.pdf');
        this.toasterService.popAsync('success', 'Success', 'Ready to download');
      },
      errors => {
        let errorMessage = 'Something wrong with error: ' +
