...
export class PaymentIndexComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private salesReportPaymentService: SalesReportPaymentService,
  ...
  getDatatable() {
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.salesReportPaymentService.indexDatatables(dataTablesParameters, this.dataFormFilter)
      ...
      columns: [
        ...
        {title: 'Payment Date', data: 'payment_date', orderable: true},
        {title: 'Payment No', data: 'code', orderable: true},
        {title: 'Customer', data: 'customer_code', orderable: true},
        {title: 'Branch', data: 'branch', orderable: true},
        {title: 'Division', data: 'division', orderable: false},
        {title: 'Sales Rep', data: 'name', orderable: true},
        {title: 'Invoice amount', data: 'invoice_amount', orderable: false},
        {title: 'Payment amount', data: 'payment_amount', orderable: false},
  ...
  print() {
    ...
    for (x of this.datasets) {
      ...
      row += '<tr>';
      row += '<td>' + i + '</td>';
      row += '<td>' + x.payment_date + '</td>';
      row += '<td>' + x.code + '</td>';
      row += '<td>' + x.customer_code + '</td>';
      row += '<td>' + x.user.branch_name + '</td>';
      row += '<td>' + x.user.division_name + '</td>';
      row += '<td>' + x.user.employee_name + '</td>';
      row += '<td>' + x.invoice_amount + '</td>';
      row += '<td>' + x.payment_amount + '</td>';
      row += '<td>' + x.payment_status + '</td>';
      row += '</tr>';
      table += row;
    }
    ...
    popupWin.document.write(`
      <html>
        ...
        <body>
          <h1 class="page-header">Report Payment</h1>
  ...
  export() {
    ...
    for (x of this.datasets) {
      ...
      row = {
        ...
        'Invoice amount': x.invoice_amount,
        'Payment amount': x.payment_amount,
        'Status': x.payment_status,
      };
      data.push(row);
    }
    this.excelService.exportAsExcelFile(data, 'payment');
  }

  dataFilter() {
    const activeModal = this.modalService.open(FilterPaymentComponent, {
      size: 'lg',
      container: 'nb-layout',
      backdrop: 'static',
    });
  ...
  exportExcel() {
    ...
    this.salesReportPaymentService.exportExcel(this.dtParams, this.dataFormFilter).subscribe(data => {
        fileSaver.saveAs(data, 'payment-report.xlsx');
  ...
  exportPdf() {
    this.toasterService.popAsync('info', 'Start Download', 'Generating, Pleasewait...');
    this.salesReportPaymentService.exportPdf(this.dtParams, this.dataFormFilter).subscribe(data => {
        console.info(`pdf data: ${data}`);
        fileSaver.saveAs(data, 'payment-report.pdf');
        this.toasterService.popAsync('success', 'Success', 'Ready to download');
      },
      errors => {
        let errorMessage = 'Something wrong with error: ' +
