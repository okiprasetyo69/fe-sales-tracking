...
@Component({
  selector: 'ngx-visit-cycle-edit',
  templateUrl: './visit-cycle-edit.component.html',
  styleUrls: ['./visit-cycle-edit.component.scss'],
})
export class VisitCycleEditComponent implements OnInit, OnDestroy {
  id: number;
    ...
    this.approvalService.show_approval(id)
      ...
      .subscribe(results => {
        ...
        errors => {
          ...
          setTimeout(() => {
            this.router.navigate(['/pages/sales/visit_cycle/index']).then();
          }, 2000);
        });
  }
  ...
  prepareCreateForm() {
    console.info(this.userDropdown);
    this.dataForm = {
      user_id: null,
      asset_id: null,
      cycle_number: null,
  ...
  saveData(formValue, nextAction) {
    ...
    this.vcService.save(dataBody, this.id)
      .pipe(untilDestroyed(this))
      .subscribe(res => {
  ...
  dataEdit() {
    this.router.navigate([`pages/sales/visit_cycle/edit/${this.id}`]).then();
  }

  loadUserDropdown() {
    this.userDropdownLoading = true;
    this.vcService.dropdownUser()
      .pipe(untilDestroyed(this))
      .subscribe(user => {
        this.userDropdownLoading = false;
        this.userDropdown = user.data.data;
      });
  }
