...
@Component({
  selector: 'ngx-collect-plan-index',
  templateUrl: './collect-plan-index.component.html',
  styleUrls: ['./collect-plan-index.component.scss'],
})
export class CollectPlanIndexComponent implements OnInit, OnDestroy {
  @ViewChild(IndexTableComponent)
  indexTable: IndexTableComponent;

  endPoint: string = '/visit/plan/collector';
  replacement: string = '/pages/collector/activities/collect_plan/index/page';
  tableData: TableData[] = [
  ...
  dataView(id) {
    this.router.navigate([`/pages/collector/activities/collect_plan/view/${id}`]).then();
  }

  dataEdit(id) {
    this.router.navigate([`/pages/collector/activities/collect_plan/edit/${id}`]).then();
  }
