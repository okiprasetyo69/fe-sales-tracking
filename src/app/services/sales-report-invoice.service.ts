...
@Injectable()
export class SalesReportInvoiceService {
  ...
  indexDatatables(payload, dataFormFilter: FormGroup): Observable<any> {
    ...
    // this.location.replaceState('pages/sales/report_invoice/index/page',
    //   'start=' + payload.start +
    //   '&length=' + payload.length +
    //   '&search=' + payload.search.value +
    //   '&order_col=' + payload.order[0].column +
    //   '&order_dir=' + payload.order[0].dir);
    this.location.replaceState('pages/collector/report_invoice/index/page',
    'start=' + payload.start +
    '&length=' + payload.length +
    '&search=' + payload.search.value +
    '&order_col=' + payload.order[0].column +
  ...
  exportExcel(payload, dataFormFilter: FormGroup): Observable<any> {
    ...
    return this.apiService.exportExcel('/sales/invoice/export', paramsExtra).map(data => data);
  }

  exportPdf(payload, dataFormFilter: FormGroup): Observable<any> {
    const paramsExtra = this.paramsFilter(payload, dataFormFilter);
    return this.apiService.exportExcel('/sales/invoice/export/pdf', paramsExtra).map(data => data);
  }
