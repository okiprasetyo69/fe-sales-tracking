...
@Injectable()
export class SalesReportOrderSalesService {
  ...
  indexDatatables(payload, dataFormFilter: FormGroup): Observable<any> {
    ...
    this.location.replaceState('pages/sales/report_order_sales/index/page',
      'start=' + payload.start +
  ...
  exportExcel(payload, dataFormFilter: FormGroup): Observable<any> {
    ...
    return this.apiService.exportExcel('/sales/request/export', paramsExtra).map(data => data);
  }

  exportPdf(payload, dataFormFilter: FormGroup): Observable<any> {
    const paramsExtra = this.paramsFilter(payload, dataFormFilter);
    return this.apiService.exportExcel('/sales/request/export/pdf', paramsExtra).map(data => data);
  }
}
