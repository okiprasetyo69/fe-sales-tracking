...
@Injectable()
export class SalesReportAlertService {
  ...
  indexDatatables(payload, dataFormFilter: FormGroup): Observable<any> {
    ...
    this.location.replaceState('pages/sales/report_alert/index/page',
      'start=' + payload.start +
  ...
  indexDatatablesCollector(payload, dataFormFilter: FormGroup): Observable<any> {
    ...
    this.location.replaceState('pages/collector/report_alert/index/page',
      'start=' + payload.start +
      '&length=' + payload.length +
      '&search=' + payload.search.value +
      '&order_col=' + payload.order[0].column +
