...
@Injectable()
export class SalesReportSalesOrderService {
  ...
  indexDatatables(payload, dataFormFilter: FormGroup): Observable<any> {
    ...
    this.location.replaceState('pages/sales/report_sales_order/index/page',
      'start=' + payload.start +
  ...
  exportExcel(payload, dataFormFilter: FormGroup): Observable<any> {
    ...
    return this.apiService.exportExcel('/sales/order/export', paramsExtra).map(data => data);
  }

  exportPdf(payload, dataFormFilter: FormGroup): Observable<any> {
    const paramsExtra = this.paramsFilter(payload, dataFormFilter);
    return this.apiService.exportExcel('/sales/order/export/pdf', paramsExtra).map(data => data);
  }
