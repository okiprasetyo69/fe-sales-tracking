...
@Injectable()
export class LogisticReportPackingSlipService {
  ...
  indexDatatables(payload, dataFormFilter: FormGroup): Observable<any> {
    ...
    this.location.replaceState('pages/logistic/report_packing_slip/index/page',
      'start=' + payload.start +
  ...
  exportExcel(payload, dataFormFilter: FormGroup): Observable<any> {
    const params = this.paramsFilter(payload, dataFormFilter);
    return this.apiService.exportExcel('/packing/slip/export', params).map(data => data);
  }

  exportPdf(payload, dataFormFilter: FormGroup): Observable<any> {
    const paramsExtra = this.paramsFilter(payload, dataFormFilter);
    return this.apiService.exportExcel('/packing/slip/export/pdf', paramsExtra).map(data => data);
  }
